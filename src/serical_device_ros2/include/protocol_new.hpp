...
constexpr int8_t FIRE_STOP = 0;
constexpr int8_t FIRE_CONTINUOUS = 1;
constexpr int8_t FIRE_SINGLE = 2;
...
struct __attribute__((packed)) RobotCtrlData
{
  ...
  int8_t target_lock = TARGET_UNLOCKED;  // 49: lock, 50: unlock
  int8_t fire_command = FIRE_STOP;
};
...
static_assert(sizeof(VisionData) == 47);
static_assert(sizeof(RobotCtrlData) == 26);
static_assert(sizeof(MsgEndInfo) == 2);
}  // namespace io
